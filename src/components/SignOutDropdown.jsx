...
export default function SignOutDropdown({ text, username = "username" }) {
    ...
    return (
        <div className="relative " onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            ...
            <div className={`absolute top-full right-0 z-10 pt-1 overflow-hidden transition-all duration-200 ease-out ${hovered ? "h-auto opacity-100" : "h-12 opacity-0"}`}>
                <div className={`flex flex-col gap-1 items-start bg-text-light px-2 pb-2 pt-1.5 rounded-xl text-sm font-medium text-text-dark`}>
                    <div className="w-full px-0.5">
                        <span className="pb-1.5 px-1 mb-1.5 w-full inline-block text-text-muted/80 border-b border-text-muted/70 text-xs tracking-wide">@{username}</span>
                    </div>
